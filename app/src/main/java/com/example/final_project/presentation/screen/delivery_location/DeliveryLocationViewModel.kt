...
class DeliveryLocationViewModel @Inject constructor(
    ...
    val locations = getDeliveryLocationsUseCase

    fun onEvent(event: DeliveryLocationEvent) {
        when (event) {
            is DeliveryLocationEvent.SelectAddressTypeEvent -> selectAddressType(address = event.addressType)
            is DeliveryLocationEvent.AddLocationEvent -> with(event) {
                addLocation(buildingName, entrance, floor, apartment, description, locationType, addressLocationName, addressLocation)
            }
            is DeliveryLocationEvent.UpdateErrorMessage -> updateErrorMessage(errorMessage = event.errorMessage)
        }
    }
